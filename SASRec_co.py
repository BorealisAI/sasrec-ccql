...
def parse_args():
    parser = argparse.ArgumentParser(description="SASRec CO.")
    ...
    parser.add_argument('--exp_id', type=str, default='SASRecCO')
...
def evaluate(model, total_step, dataset):
    ...
    for i in range(len(topk)):
        ...
        if topk[i] == 20 and total_step >= 10:
            ...
            print('Reward: {:.2f}'.format((best_ckpt['max_reward'])),
...
class SASRecnetwork(torch.nn.Module):
    ...
    def forward(self, state_seq, len_state):
        ...
        # There is no equivalent of tf.gather_nd in pytorch afaik
        # For now we do it in a loop.
        layer_slices = []
...
if __name__ == '__main__':
    ...
    # InfoNCE, InstanceInfoNCE, Hierarchical
    ...
    print('Model : SASRec Contrastive')
    ...
    evaluate(SASRec1, total_step, dataset)
        ...
        for j in range(num_batches):
            ...
            if total_step < switch_interval:
                ...
                if contrastive_objective == "InfoNCECosine":
                    contrastive_loss = utils.info_nce_cosim_loss(main_model_current_state,
                                                            logits_aug_state,
                                                            total_step,
                                                            console)
                elif contrastive_objective == "InfoNCE":
                    main_model_current_state = main_model_current_state.unsqueeze(-1)
                    logits_aug_state = logits_aug_state.unsqueeze(-1)
                    contrastive_loss = utils.contrastive_loss(main_model_current_state,
                                                                        logits_aug_state)
                elif contrastive_objective == "Hierarchical":
                    main_model_current_state = main_model_current_state.unsqueeze(-1)
                    logits_aug_state = logits_aug_state.unsqueeze(-1)
                    contrastive_loss = utils.hierarchical_contrastive_loss(main_model_current_state,
                                                                        logits_aug_state)
                mainQN.opt.zero_grad()
                loss = mainQN.celoss1(predictions, actions)

                final_loss = torch.mean(loss + q_loss + contrastive_loss)
                final_loss.backward()
                mainQN.opt.step()
                ...
                final_loss = torch.mean(loss_multi + q_loss + contrastive_loss)
                final_loss.backward()
                mainQN.opt2.step()
